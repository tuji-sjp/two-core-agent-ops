...
- `/business/overview` — 业务全景概览（BusinessOverview），浅色主题全景大屏（bg `#fff`，blue `#3b82f6` 主色调）。整体容器：`height: 100%`, flex column, 白色背景, borderRadius 16, border `1px solid #e5e7eb`, boxShadow `0 1px 3px rgba(0,0,0,0.05)`，无页面滚动。**5 大板块一屏 3 列 flex 分割展示**：
  ...
  - **板块 1 数据资产总览**（DataAssetsSection，flex:1）：理赔/核保 tab 切换，4 类卡片（影像数据/体检报告/保单信息/案件信息），每项展示总量（blue #3b82f6）+ 使用量（green #52c41a），**2 列 grid 布局**。
  ...
  - **板块 5 知识调用排行**（KnowledgeSection，右列下）：从 mock-data 的 claimsKnowledge/underwritingKnowledge 提取 Top 10 + category，**理赔/核保 tab 切换**，每行含排名徽章（前 3 名 blue 高亮）+ 文档名称 + 彩色分类标签 pill + 调用次数。
  - **DashboardHeader**：顶部标题栏，白色 bg + 蓝色（#3b82f6）左右装饰竖条 + "两核智能体 · 业务全景大屏"。
  - **sectionLabel** 装饰：4px 宽 × 12px 高蓝色（#3b82f6）竖条，borderRadius 10, marginRight 10。
