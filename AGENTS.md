...
<!-- 2026-06-15: Skills市场"发布新Skill"弹窗功能增强：点击"发布新Skill"按钮触发固定定位模态框（position: fixed, zIndex 1000, 半透明黑色遮罩 background rgba(0,0,0,0.5)，白色内容区 borderRadius 16 maxWidth 700px maxHeight 85vh overflow auto padding 24）。表单字段包含：Skill名称（必填红色星号标记）、概述（textarea）、功能特性（动态数组输入项，支持添加/删除，每项带序号 + 输入框 + DeleteOutlined删除按钮）、分类标签（4个可选tag：OCR识别/数据采集/立案定责/核保评估，点击切换选中状态，选中态蓝色背景#eff6ff + 蓝色边框#3b82f6）、SKILL.md（大文本框 monospace字体）、API参考（动态数组，每项含接口名monospace字体/描述/返回字段三个输入框 + 删除按钮，底部"+ 添加接口"虚线按钮）、错误码（动态数组，每项含错误码monospace/描述/解决方案三个输入框 + 删除按钮，底部"+ 添加错误码"虚线按钮）。**弹窗交互优化**：打开弹窗时恢复上次保存的草稿（`setNewSkill(savedDraft)`），点击关闭按钮或遮罩层执行 `handleCancel` 回退到上次保存状态。底部按钮组从"取消+发布"改为"取消+保存+提交审核"三按钮布局：**取消按钮**调用 `handleCancel` 关闭弹窗并回退表单；**保存按钮**执行 `handleSave` 校验同名 → 保存详情页内容到 skillContentMap（不创建卡片）→ 更新 savedDraft 状态；**提交审核按钮**执行 `handleSubmit` 完整校验 → 保存内容 + 创建卡片（allSkills + mySkills）→ 清空表单和 savedDraft → 跳转详情页。**新增校验逻辑**：`validateForm()` 函数统一校验所有必填项（名称/概述/features/categories/skillMd/versions/apis/errors），`saveContent()` 函数负责写入 skillContentMap，`createCard()` 函数负责创建 allSkills/mySkills 卡片条目。涉及文件：`src/pages/shared/skills/index.tsx` -->

<!-- 2026-06-15: SkillDetail 页面编辑功能增强：顶部导航栏新增 EditOutlined/CloseOutlined/DeleteOutlined 图标导入，新增 isEditing/editContent 状态管理，实现 handleEdit/handleCancel/handleSubmit/updateField 编辑函数，添加编辑/取消按钮UI（位于导航栏下方），支持对 overview/features/versions/skillMd/apis/errors 字段的实时内联编辑。编辑模式下所有章节边框变为蓝色 2px 高亮（`border: isEditing ? '2px solid #3b82f6' : '1px solid #e8e8e8'`）。概述章节：overview 使用多行 textarea（minHeight 80px, resize vertical），features 提供列表编辑器（每项带序号输入框 + DeleteOutlined 删除按钮 + 底部"添加功能"虚线按钮），分类标签改为多选数组（点击切换选中状态，选中态蓝色背景#eff6ff + 蓝色边框#3b82f6 + fontWeight 600，未选中态白色背景 + 灰色边框#d9d9d9）。版本历史章：每项三个输入框（版本 monospace 字体 width 100px / 日期 width 130px / 更新内容 flex 1）+ DeleteOutlined 删除按钮，底部"添加版本"按钮自动生成新版本条目；渲染时自动按语义版本号排序（v0.x 内测版在前，v1.x+ 正式版在后，移除 v/V 前缀后按 major.minor.patch 数值比较）。SKILL.md 章节：大型 textarea（minHeight 400px, monospace 字体, background #f9fafb）编辑 markdown 源码。API参考和错误码章节：每项四个输入框（接口名 monospace width 180px / 描述 width 160px / 返回字段 monospace flex 1 或 错误码 width 140px / 描述 width 180px / 解决方案 flex 1）+ DeleteOutlined 删除按钮，底部"添加接口"/"添加错误码"按钮新增空条目。**编辑状态底部按钮**：仅在编辑模式显示，flex 布局 gap 12，左侧"保存"按钮（白色背景 #fff + 灰色边框 #d9d9d9，调用 handleSubmit 保存并退出编辑）+ 右侧"提交审核"按钮（蓝色背景 #3b82f6 + boxShadow，调用 handleSubmit 后提示已提交审核）。**版本历史右侧独立栏**：从概述章节中抽离，改为两栏布局（左侧主内容 flex:1 + 右侧版本历史 aside 宽 380px sticky 定位，maxHeight calc(100vh - 140px) overflow-y auto）。版本卡片式布局（每个版本 borderRadius 8, padding 14, marginBottom 10, border 1px solid #ececec），展示版本号（Consolas 字体 fontSize 15）+ 更新内容 + 发布者/日期 + 回滚按钮（仅非当前版本可见，点击触发固定定位确认弹窗）。当前版本标识：version 匹配 `latestContent.currentVersion || latestVersion` 时回滚按钮隐藏（visibility: hidden）。选中态高亮：点击卡片背景变 #eff6ff + 边框 #bfdbfe。编辑模式下版本历史栏也变为蓝色 2px 边框高亮。**currentVersion 字段**：SkillContent 接口新增 `currentVersion?: string` 存储回滚目标版本，回滚确认后更新该字段并 navigate 到对应版本 URL。涉及文件：`src/pages/shared/skill-detail/index.tsx`, `src/pages/shared/skills/index.tsx` -->
