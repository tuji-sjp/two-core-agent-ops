...
- **影像预览弹窗图像区域自适应** ✅ 已优化（2026-06-23）：
  - 单图预览：图像区域占左侧容器 80%，保持 3:4 比例，根据弹窗高度自适应
  - 双图对比：图像区域占左侧容器 90%，每张图保持 3:4 比例
  - 所有图像（大图、缩略图）统一 3:4 比例
- **缩略图区域重构** ✅ 已优化（2026-06-23）：
  - 缩略图改为纵向布局（上图下文），下方添加 img_id 显示（monospace 字体，选中态蓝色）
  - 外层容器固定高度 130px，每张缩略图高度 92px（3:4 比例 → 宽 69px）
  - SVG 图标从 16 调整到 22 适配新尺寸
- **重复影像交互优化** ✅ 已完成（2026-06-23）：
  - duplicateGroup 改为包含同分类下所有影像（保证缩略图 ≥2 张）
  - 右图与缩略图选中态同步更新（点击缩略图/导航时右图同步切换）
  - 新增 `getDuplicateIndex` 辅助函数，统一计算影像在同分类中的位置
  - `openPreview` / prev / next 导航都使用该函数同步 selectedDuplicateIndex 状态
- **切割红框优化** ✅ 已优化（2026-06-23）：
  - 双图对比区左图也添加切割红框（与单图预览逻辑一致）
  - 所有带"切割"标签的预览影像，红框从 2~4 个改为固定 1 个
- **弹窗标题格式优化** ✅ 已完成（2026-06-23）：
  - 标题格式改为：影像类别名（第一行）+ 序号/总数 - img_id（第二行）
  - 去掉副标题的括号，统一使用 `previewIndex + 1 / filteredImages.length` 格式
- **分组标题格式优化** ✅ 已完成（2026-06-23）：
  - 已分组（嵌套）：`已分组 · 一级 · 二级 - 分类名`（如"已分组 · 住院组 · 病历组 - 出院小结"）
  - 待分组/无需分组（平铺）：`分组名 - 分类名`（如"待分组 - 出院小结"）
- **全部视图排序优化** ✅ 已完成（2026-06-23）：
  - "全部"分类下展示的影像按 CATEGORIES 列表从上到下的顺序排序
  - 所有模式（全部/单分类/分组）都按分类顺序排序
